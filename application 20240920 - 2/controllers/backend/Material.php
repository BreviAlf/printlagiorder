...
class material extends CI_Controller {
	...
	function index()
	{
		...
		$data['title'] = 'SISPRINT - Product material';

		//sidebar prop

		$data['sidebar_active'] = "sidebar_material";
		$data['collapse_active'] = "product-nav";

		//data
		$this->load->library('pagination');
		$offset = $this->uri->segment(4);
		$config['base_url'] = site_url().'/backend/material/index';
		$config['total_rows'] = count($this->product_model->Getmaterial('all',FALSE,FALSE,FALSE));
		$config['uri_segment'] = '4';

		$this->product_model->Getmaterial();
		...
		$data['offset'] = $offset;
		$data['template'] = 'material/index';
		$data['arr_cat'] = $this->product_model->Getmaterial('all',FALSE,$config['per_page'],$offset);
	...
	public function add()
	{
		...
		{	
			...
			$data['title'] = 'SISPRINT - Add material';
			$data['template'] = 'material/add';
			$this->load->view('backend/index',$data);
		...
		else
		{
			$data = array
				...
				'material_name'				=> $this->input->post('material_name'),  //req
				'material_gsm'  			=> $this->input->post('material_gsm'),
				'material_price'  			=> $this->input->post('material_price')
				);

			$this->product_model->CreateMaterial($data);
			CreateLog('INFO','add save material');
	...
	public function edit()
	{
		...
		{	
			...
			$data['row_data'] = $this->product_model->Getmaterial(FALSE,$material_id,FALSE,FALSE);
			//sidebar prop
			$data['sidebar_active'] = "sidebar_material";
			$data['collapse_active'] = "product-nav";

			$data['title'] = 'SISPRINT - Edit material - '.$data['row_data']->material_name;
			$data['template'] = 'material/edit';
			$this->load->view('backend/index',$data);
		...
		{
			$material_id = $this->uri->segment(4);
			$data = $this->product_model->Getmaterial(FALSE,$material_id,FALSE,FALSE);
			...
			$this->product_model->Updatematerial($material_id,$data);
			CreateLog('INFO','Save Edit material :'.$material_id);
			redirect('backend/material');
		}	
	}
	...
	public function delete()
	{
		...
		$data = $this->product_model->Getmaterial(FALSE,$prod_id,FALSE,FALSE);
		...
		CreateLog('INFO','Delete material :'.$prod_id.' Delete image'.$data->cat_img_url);
        */

		$this->product_model->Deletematerial($prod_id);
