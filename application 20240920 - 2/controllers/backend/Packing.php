...
class Packing extends CI_Controller {
	...
	function index()
	{
		...
		$data['title'] = 'SISPRINT - SPK New Packing List';
		//sidebar prop
		$data['sidebar_active'] = "sidebar_pack";
		$data['collapse_active'] = "pack-nav";
		
		CreateLog('INFO','load page');
		$data['urut'] = $this->uri->segment(4);
		$data['template'] = 'packing/add';
	...
	public function add()
	{
		$this->form_validation->set_rules('pack_kurir_id', 'Nama Kurir','required');
		
		if($this->form_validation->run() == FALSE)
		{	
		
			$data['title'] = 'SISPRINT - NEW Packing List';
			//sidebar prop
			$data['sidebar_active'] = "sidebar_pack";
			$data['collapse_active'] = "pack-nav";
		...
		{
			$pack_uid = GenUidPack('tb_packing',FALSE);

			$data = array
				(
					'pack_uid'					=> $pack_uid,
					'pack_no'					=> 'PACK-'.$pack_uid,
					'pack_kurir_id'				=> $this->input->post('pack_kurir_id'),
					'pack_kurir_code'			=> getDataTableById('tb_kurir','kurir_code','kurir_id',$this->input->post('pack_kurir_id')),
					'pack_kurir_name'			=> getDataTableById('tb_kurir','kurir_name','kurir_id',$this->input->post('pack_kurir_id')),
					'pack_user_id'				=> $this->session->userdata('user_id'),
					'pack_status'				=> 'Created'
				);

			CreateLog('INFO','insert data');
			$this->db->insert('tb_packing',$data);
			$pack_id = $this->db->insert_id();
			redirect('backend/packing/detail/'.$pack_id);
		}	
	}
	...
	{	
		...
		$data['data_pack'] = $this->db->get('tb_packing')->row();

		if($data['data_pack']->pack_status == 'Created'){
			$data['input'] = "";
		}else{
			$data['input'] = "disabled";
		}
		
		$data['title'] = 'SCAN INVOICE PACK NO : '. $data['data_pack']->pack_no;
		//sidebar prop
		$data['sidebar_active'] = "sidebar_pack";
		$data['collapse_active'] = "pack-nav";
		...
		$this->db->where('pack_det_pack_id',$pack_id);
		$this->db->order_by('pack_det_created_date','desc');
		$data_pack_det = $this->db->get('tb_packing_detail')->result_array(); 
		$data['arr_pack_det'] = $data_pack_det;
		
		CreateLog('INFO','load page');
		$data['pack_id'] = $this->uri->segment(4);
		$data['notif'] = "";
		$data['total_pack'] = count($data_pack_det);
		$data['template'] = 'packing/add_pack';
		$this->load->view('backend/index',$data);
	}
	...
	public function AjaxInsertInv()
	{
		$pack_id = $this->input->post('pack_id');
		$inv_mp = $this->input->post('inv_mp');
		
		// get data spk by invoice no ROW just need 1 row only
		$this->db->where('spk_inv_mp',$inv_mp);
		$data_spk = $this->db->get('tb_spk')->row();

		// get data spk no resi ROW just need 1 row only
		$this->db->where('spk_no_resi',$inv_mp);
		$data_spk_resi = $this->db->get('tb_spk')->row();

		// spk found
		if($data_spk || $data_spk_resi)
		{

			if($data_spk_resi){
				$this->db->where('spk_no_resi',$inv_mp);
				$data_spk_resi = $this->db->get('tb_spk')->row();
				$inv_mp = $data_spk_resi->spk_inv_mp;
			...
			$this->db->where('pack_det_inv_mp',$inv_mp);
			$data_pack_check = $this->db->get('tb_packing_detail')->result_array(); 

			if(count($data_pack_check) > 0)
			{

				$notif = '<div class="alert alert-warning bg-warning text-dark border-0 alert-dismissible fade show" role="alert">
				<h3>INVOICE : '. $inv_mp. ' <hr><strong>SUDAH SCAN!!</strong></h3>
				
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button>
              </div>
			  <audio id="myAudio">
					<source src="'.site_url().'/assets/audio/warning.mp3" type="audio/mpeg">
			...
			else
			{
			  ...
			  ';
				...
				// update tb_spk
				// check done status
				if($data_spk->spk_status == 'Done'){
					// update Packing ONlY
					$data_update = array
					(
						'spk_status'			=> 'Packing',
						'spk_time_packing'		=> $time_now
					);

				}else{
					// update Done and Packing
					$data_update = array
					(
						'spk_status'			=> 'Packing',
				...
				$this->db->where('spk_inv_mp',$inv_mp);
				$this->db->update('tb_spk',$data_update);
				...
				$this->db->where('batch_spk_det_spk_inv_mp',$inv_mp);
				$this->db->update('tb_batch_spk_detail',$data_update2);

				CreateLog('INFO','update data - tb_spk - invoice MP : '.$inv_mp );

				//insert tb_packing_detail
				$data = array
				(
					'pack_det_pack_id'		=> $pack_id,
					'pack_det_inv_mp'		=> $inv_mp
				);

				CreateLog('INFO','insert data - tb_packing_detail');
				$this->db->insert('tb_packing_detail',$data);
		...
		else
		{
			...
			<h3>INVOICE : '. $inv_mp. ' <hr><strong>TIDAK ADA!!</strong></h3>
			<button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="Close"></button>
		  </div>
		  <audio id="myAudio">
	...
	public function AjaxDelDetId()
	{
		$pack_id 		= $this->input->post('pack_id');
		$pack_det_id 	= $this->input->post('pack_det_id');

		// del tb_batch_spk_detail by batch_spk_det_batch_spk_id
		$this->db->where('pack_det_id',$pack_det_id);
		$this->db->delete('tb_packing_detail');
	...
	function update()
	{
		...
		$this->db->where('pack_det_pack_id',$pack_id);
		$arr_det_pack = $this->db->get('tb_packing_detail')->result_array();

		foreach($arr_det_pack as $row_det_pack)
		{
			$updateArray[] = array(
				'spk_inv_mp'			=> $row_det_pack['pack_det_inv_mp'],
				'spk_status' 			=> 'Delivered',
				'spk_time_delivered'  	=> $time_now
			);
		}

		$this->db->update_batch('tb_spk',$updateArray, 'spk_inv_mp'); 

		CreateLog('INFO','update batch data id '.$pack_id);

		$data = array
				(
					'pack_date_done'	=> $time_now,
					'pack_status'		=> 'Done',
				);
		$this->db->where('pack_id',$pack_id);
		$this->db->update('tb_packing',$data);
		...
		if($flag == 's'){
			redirect('backend/packing/');
		}else if($flag == 'h'){
			redirect('backend/packing/history');
		}else{
			redirect('backend/packing/cetak_pack/'.$pack_id);
		}
		
	}


	function history($offset = FALSE, $search_q = FALSE)
	{
		if(!$offset){$offset=0;}
		if($this->input->post('search_q') || $search_q!=null){
			...
			$data['arr_pack'] = $this->db->query('SELECT * FROM tb_packing WHERE  
				pack_no LIKE "%'.$teks.'%" OR 
				pack_kurir_name LIKE "%'.$teks.'%" OR 
				pack_date_created LIKE "%'.$teks.'%" OR 
				pack_date_done LIKE "%'.$teks.'%" OR 
				pack_status LIKE "%'.$teks.'%" 
				ORDER BY pack_date_created DESC LIMIT 10 OFFSET '. $offset.' ')->result_array();
				 
			$data['row_count'] = $this->db->query('SELECT * FROM tb_packing WHERE  
				pack_no LIKE "%'.$teks.'%" OR 
				pack_kurir_name LIKE "%'.$teks.'%" OR 
				pack_date_created LIKE "%'.$teks.'%" OR 
				pack_date_done LIKE "%'.$teks.'%" OR 
				pack_status LIKE "%'.$teks.'%" 
				ORDER BY pack_date_created DESC')->num_rows();
				
			$data['current_offset'] = $offset;
			$data['sidebar_active'] = "sidebar_pack_history";
			$data['collapse_active'] = "pack-nav";
			$data['title'] = 'SISPRINT - Packing LIST';
			$data['template'] = 'packing/history';
			$this->load->view('backend/index',$data);
		
		} else {
		
			$qr = $this->db->select('*');
			$qr = $this->db->from('tb_packing');
			$qr = $this->db->limit(10,$offset);
			$qr = $this->db->order_by('pack_date_created','desc');
			$qr = $this->db->get();
			$data['arr_pack'] = $qr->result_array();
			$data['row_count'] = $this->db->query('SELECT * FROM tb_packing ')->num_rows();
			$data['current_offset'] = $offset;
			$data['search_q'] = '';
	...
	function cetak_pack()
	{
		...
		// QR DATA BATCH
		$this->db->where('pack_id',$pack_id);
		$row_pack = $this->db->get('tb_packing')->row();

		
		// QR DATA SPK
		$qr = $this->db->select('*');
		$qr = $this->db->from('tb_packing_detail');
		$qr = $this->db->where('pack_det_pack_id',$pack_id);
		$qr = $this->db->order_by('tb_packing_detail.pack_det_inv_mp','asc');
		$qr = $this->db->get();
		$arr_pack = $qr->result_array();

		$file_pdf = $row_pack->pack_no;
		...
		*/
		...
		$pdf->Cell(1.5, 0.5, 'Kurir',1,'LR','L');
		$pdf->SetFont('helvetica','B',8);
		$pdf->Cell(2.5, 0.5, $row_pack->pack_kurir_name,1,'LR','L');
		$pdf->Cell(0.4, 0.5, '',0,'LR','L');
		$pdf->Ln();
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell(1.5, 0.5, 'Created',1,'LR','L');
		$pdf->SetFont('helvetica','B',7);
		$pdf->Cell(2.5, 0.5, $row_pack->pack_date_created,1,'LR','L');
		$pdf->Cell(0.4, 0.5, '',0,'LR','L');
		$pdf->Ln();
		$pdf->SetFont('helvetica','',7);
		$pdf->Cell(1.5, 0.5, 'UID',1,'LR','L');
		$pdf->SetFont('helvetica','B',7);
		$pdf->Cell(2.5, 0.5, $row_pack->pack_uid,1,'LR','L');
		$pdf->Cell(0.38, 0.5, '',0,'LR','L');
		$pdf->SetFont('helvetica','B',10);
		$pdf->Cell(2, 0.5, 'PACK NO : '. $row_pack->pack_no,0,'LR','L');
		$pdf->Ln();
		$pdf->SetFont('helvetica','',8);
		$pdf->Cell(1.5, 0.5, 'JML',1,'LR','L');
		$pdf->SetFont('helvetica','B',8);
		$pdf->Cell(2.5, 0.5, count($arr_pack),1,'LR','L');
		$pdf->Ln();


		
		$pdf->Code128(5,0.5,$row_pack->pack_no,5,1);
		$pdf->Ln();
		
		

		$pdf->SetFont('helvetica','B',6);
		$pdf->Cell(3, 0.3, 'NO INVOICE MP',1,'LR','L');
		$pdf->SetFont('helvetica','B',6);
		$pdf->Cell(4, 0.3, 'Scan Check',1,'LR','L');
		$pdf->SetFont('helvetica','B',6);
		$pdf->Cell(2, 0.3, 'Check',1,'LR','L');
		$pdf->Ln();

	
		foreach ($arr_pack as $row_pack)
		{
			$pdf->SetFont('helvetica','',6);
			$pdf->Cell(3, 0.3, $row_pack['pack_det_inv_mp'],1,'LR','L');
			$pdf->SetFont('helvetica','',6);
			$pdf->Cell(4, 0.3, $row_pack['pack_det_created_date'],1,'LR','L');
			$pdf->SetFont('helvetica','',6);
			$pdf->Cell(2, 0.3,"",1,'LR','L');
			$pdf->Ln();
		}
		foreach ($arr_pack as $row_pack)
