...
class Category extends CI_Controller {
	...
	function index()
	{
		...
		$data['title'] = 'SISPRINT - Product Category';

		//sidebar prop

		$data['sidebar_active'] = "sidebar_category";
		$data['collapse_active'] = "product-nav";

		//data
		$this->load->library('pagination');
		$offset = $this->uri->segment(4);
		$config['base_url'] = site_url().'/backend/category/index';
		$config['total_rows'] = count($this->product_model->GetCategory('all',FALSE,FALSE,FALSE));
		$config['uri_segment'] = '4';

		$this->product_model->GetCategory();
		...
		$data['arr_cat'] = $this->product_model->GetCategory('all',FALSE,$config['per_page'],$offset);
	...
	public function add()
	{
		...
		{	
			...
			$data['title'] = 'SISPRINT - Add Category';
		...
		else
		{
			...
			$cat_img_url 			= ($_FILES['file_image']['name']? $this->upload_file('file_image') : "");
			$cat_img_size_guide_url 	= ($_FILES['file_image_size']['name']? $this->upload_file('file_image_size') : "");
			...
			$data = array
				...
				'cat_img_size_guide_url'=> $cat_img_size_guide_url,
				'cat_status'			=> $this->input->post('cat_status'),

				);

			$this->product_model->CreateCategory($data);
			CreateLog('INFO','add save category');
	...
	public function edit()
	{
		...
		{	
			...
			$data['row_data'] = $this->product_model->GetCategory(FALSE,$cat_id,FALSE,FALSE);
			//sidebar prop
			$data['sidebar_active'] = "sidebar_category";
			$data['collapse_active'] = "product-nav";

			$data['title'] = 'SISPRINT - Edit Category - '.$data['row_data']->cat_name;
		...
		{
			$cat_id = $this->uri->segment(4);
			$data = $this->product_model->GetCategory(FALSE,$cat_id,FALSE,FALSE);

			if($_FILES['file_image']['name']){
				unlink($data->cat_img_url);
			}
			
			if($_FILES['file_image_size']['name']){
				unlink($data->cat_img_size_guide_url);
			}

			$cat_img_url = ($_FILES['file_image']['name']? $this->upload_file('file_image') : $data->cat_img_url);
			$cat_img_size_guide_url = ($_FILES['file_image_size']['name']? $this->upload_file('file_image_size') : $data->cat_img_size_guide_url);
			...
			$this->product_model->UpdateCategory($cat_id,$data);
			CreateLog('INFO','Save Edit Category :'.$cat_id);
			redirect('backend/category');
		}	
	}
	...
	public function edit_prod_desc()
	{
		...
		if($save_to_all == 1):
			...
			$this->product_model->UpdateCategory($cat_id,$data);
			CreateLog('INFO','Save Edit Category on cat_prod_desc :'.$cat_id);
	...
	public function delete()
	{
		...
		$data = $this->product_model->GetCategory(FALSE,$prod_id,FALSE,FALSE);

		//echo $_SERVER['DOCUMENT_ROOT'].'/'.$data->prod_img_mockup_url;
		//exit();
		unlink($data->cat_img_url);
		CreateLog('INFO','Delete Category :'.$prod_id.' Delete image'.$data->cat_img_url);
		$this->product_model->DeleteCategory($prod_id);
