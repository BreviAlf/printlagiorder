<script type="text/javascript">
  ...
  function AddVarian() {
    // alert(document.getElementById("prod_var_child_parent").checked);
  ...
  function child_modal_template_ver2(parent_SKU_Varian, Child_name, prod_var_parent_id) {
    ...
    document.getElementById("Child_Checkbox").hidden = false;
    document.getElementById("Child_Checkbox").disabled  = false;
    document.getElementById("prod_var_child_Child").disabled = false;
    document.getElementById("prod_var_child_Child").checked = true;
    // document.getElementById("Child_Checkbox_Label").hidden = false;
    
    document.getElementById("prod_var_parent_id").disabled  = false;
    document.getElementById("prod_var_parent_id").readOnly  = true;
    document.getElementById("prod_var_parent_id").value = parent_SKU_Varian;

    
    $.ajax({  
      url: '<?php echo site_url() . 'backend/product/nama_produk_child';?>',
      cache: false,
      type: 'POST',
      data: {
      ...
      success: function(result) {
        Child_name = Child_name + "_C0" + result
        document.getElementById("prod_var_sku_var").value = Child_name;
      },
      failure: function(errMsg) {
        alert(errMsg);
      }
  ...
  function generateString2(prod_var_parent_id) {
  ...
  function default_modal_template() {
    
    document.getElementById("modal_title").innerText = "Tambah Varian";
    // document.getElementById("prod_var_sku_var").readOnly = false;
    document.getElementById("Parent_Checkbox").hidden = false;
    document.getElementById("Parent_Checkbox").disabled  = false;
    document.getElementById("Parent_Checkbox_Label").hidden = false;

    document.getElementById("Child_Checkbox").hidden = true;
    document.getElementById("Child_Checkbox").disabled  = true;
    document.getElementById("Child_Checkbox_Label").disabled = true;
    // document.getElementById("Child_Checkbox_Label").hidden = true;

    document.getElementById("prod_var_parent_id").disabled  = true;
  }
...
function parent_toggle(){
  if(document.getElementById("prod_var_child_parent").checked){

    document.getElementsByName("prod_var_print_side")[0].disabled = true;
    ...
    for (var i = 0; i < finishing_checks.length; i++) {
      finishing_checks[i].disabled = true;
    }

    teks = `     
    <select name="prod_var_print_side" class="form-select">
    <option value="No Print">No Print</option>
    </select>

    <select name="prod_var_lamination" class="form-select">
    <option value="Tanpa Laminasi">Tanpa Laminasi</option>
    </select>

    <select name="prod_var_lamination_side" class="form-select" >
    ...
    // <input name="cfin[]"  value="No Cut"> </input>


  }else{
    document.getElementById("right_hidden").innerHTML = ``;
    document.getElementsByName("prod_var_print_side_disabled")[0].name = "prod_var_print_side";
  ...
  <div class="col-lg-12">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="tab-content mt-2 pt-2" id="borderedTabContent">
          <div class="tab-pane fade active show" id="bordered-info" role="tabpanel" aria-labelledby="info-tab">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#AddVarianModal" onclick="default_modal_template(); default_modal()">
                Add Varian
              </button>
              <div id="data_varian">
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modal_title">Tambah Varian</h5>
      ...
      <div class="modal-body">
        <form id="add_varian" action="<?php echo site_url() ?>backend/product/AjaxAddVarian" method="post" class="row g-3 needs-validation" enctype="multipart/form-data" novalidate>
          <div id="right_hidden" hidden></div>
          <div class="col-lg-6">
            <div class="row mb-3">
            ...
            <div class="row mb-3">
              <label for="inputText" class="col-sm-3 col-form-label">Kelipatan</label>
              <div class="col-sm-9">
                <input type="text" type="text" name="prod_var_kel" id="prod_var_kel" class="form-control">
              </div>
            </div>
            <div class="row mb-3">
              <label for="inputText" class="col-sm-3 col-form-label">Multiply (Min)</label>
              <div class="col-sm-9">
                <input type="text" type="text" name="prod_var_multiply" id="prod_var_multiply" class="form-control">
              </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3" id="Parent_Checkbox_Label">
              <label for="spk_material_name" class="col-sm-3 col-form-label" style="padding-top:0;">Produk Khusus</label>
              <div class="col-sm-9">
                <div class="form-check form-switch" id="Parent_Checkbox">
                  <input class="form-check-input" type="checkbox" onclick="parent_toggle()" id="prod_var_child_parent" name="prod_var_child_parent" >
                  <label class="form-check-label" for="prod_var_child_parent">Produk memiliki beberapa bahan</label>
                </div>
              </div>
            </div>

            
            <div class="row mb-3 kontrol_parent_id" id="parent_id_input" hidden>
              <label for="inputText" class="col-sm-3 col-form-label">PARENT ID</label>
              <div class="col-sm-9">
                <input type="text" type="text" name="prod_var_parent_id" id="prod_var_parent_id" class="form-control">
              </div>
            </div>
            <div class="row mb-3" id="Child_Checkbox_Label" hidden>
              <label for="spk_material_name" class="col-sm-3 col-form-label" style="padding-top:0;" >Add Bahan</label>
              <div class="col-sm-9">
                <div class="form-check form-switch" id="Child_Checkbox">
                  <input class="form-check-input" type="checkbox" id="prod_var_child_Child" name="prod_var_child_Child" >
                  <label class="form-check-label" for="prod_var_child_Child">Centang jika Merupakan Child</label>
                </div>
              </div>
            </div>
          ...
          <div class="col-lg-6" id="modal_right_half">
          <div class="row mb-3">
