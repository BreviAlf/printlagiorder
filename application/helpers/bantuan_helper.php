...
// ('tb_paper_size','paper_size_name','paper_size_id',$row_var['prod_var_paper_size_id'])
...
function DropdownMaterial($selected,$name,$change_event)
{
	...
	$ci->db->order_by('material_code','asc');
...
//_Print
function CountSPKDone_Print($batch_spk_id)
{
	$ci=& get_instance();
	$qr = $ci->db->query("SELECT count(batch_spk_det_batch_spk_id) 
	AS count_spk FROM tb_batch_spk_detail where batch_spk_det_spk_id = $batch_spk_id AND (batch_spk_det_print!='NULL' OR batch_spk_det_print != '0000-00-00 00:00:00')");
	$count = $qr->row()->count_spk;
	return $count;
}
