...
class Spk extends CI_Controller {
	...
	public function AjaxSearchBySKUVar()
	{
		...
		// get product category
 		$cat_name = getDataTableById('tb_category','cat_name','cat_id',$row_prod->prod_cat_id);

		$prod_check_visual = 'none';

		if($row_prod->prod_check_visual == 1){
			$prod_check_visual = 'block';
		}

		if($qr_prod_var->num_rows()>0)
		{
			...
			$data = array(
				...
				'prod_check_visual'	=> $prod_check_visual,
				'spk_prod_mp_sku'	=> $sku_induk,
				'spk_instruction'	=> $data_fin
				);
		}else{
			$d_material 	= DropdownMaterial(FALSE, 'spk_material_id',FALSE);
			$d_paper_size 	= DropdownPaperSize(FALSE, 'spk_paper_size_id',FALSE);
			$d_print_side 	= DropdownPrintSide(FALSE, 'spk_print_side');
			$d_laminasi 	= DropdownLaminasi(FALSE, 'spk_lamination');
			$d_cutting 		= DropdownCutting(FALSE, 'spk_cutting');
			$d_spk_type 	= DropdownSPKType(FALSE, 'spk_type');
			$data = array(
				'stat'			=> 0,
				'prod_name'		=> '',
				'prod_name_mp'	=> '',
	...
	function cetak_spk()
	{
		...
		*/
		...
		$pdf->SetFont('helvetica','',6);
		$pdf->Cell(2, 0.5,$row_spk->spk_inv_mp.''.$cust_name,0,'LR','L');
		...
		$pdf->Code128(5,1.3,$row_spk->spk_no,5,0.8);
		$pdf->Ln(1);
		
		$pdf->Code128(5,2.5,$row_spk->spk_inv_mp,5,0.6);
		$pdf->Ln(-0.7);
		...
		*/
		...
		$pdf->Cell(2, 0.4, $row_spk->spk_inv_mp.''.$cust_name,0,'LR','L');
		...
		$pdf->Code128(5,9.8,$row_spk->spk_inv_mp,5,0.5);
		$pdf->Ln(-0.5);
