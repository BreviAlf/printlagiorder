...
class Product extends CI_Controller {
	...
	public function edit()
	{
		...
		{	
			...
			$this->db->order_by('prod_var_parent_id','asc');
	...
	public function AjaxAddVarian()
	{
		...
		$parent_flag = NULL;
		$parent_id = NULL;
		if($this->input->post('prod_var_child_parent') == true ){
			$parent_flag = 1;
			$parent_id = NULL;
		} 
		if($this->input->post('prod_var_child_Child') == true ) {
			$parent_flag = 2;
			$parent_id = $this->input->post('prod_var_parent_id');
		} 
		...
		(
			...
			'prod_var_parent_flag'			=> $parent_flag,
			'prod_var_parent_id'  => $parent_id
		);

		// 'prod_var_parent_id'			=> 0
		...
		if($parent_flag == 1 ){
			...
			// UPDATE SQL prod_var_parent_id = $last_id  where prod_var_id = $last_id 
			$data = array(
        'prod_var_parent_id' => $last_id
			);

			$this->db->where('prod_var_id', $last_id);
	...
	public function AjaxDelVar()
	{
		...
		$parent_check = $this->db->get('tb_prod_var')->row()->prod_var_parent_flag;
		
		if($parent_check == 1){
			$this->db->where('prod_var_parent_id',$prod_var_id);
	...
	public function AjaxCopyVar()
	{
		...
		(
			...
			'prod_var_parent_flag'				=> $row_cp->prod_var_parent_flag,
			'prod_var_parent_id'				=> $row_cp->prod_var_parent_id
		);

		CreateLog('INFO','insert data');
		$this->db->insert('tb_prod_var', $data);

		if($row_cp->prod_var_parent_flag == 1){
			$last_id = $this->db->insert_id();
	...
	public function nama_produk_child(){
		$prod_var_parent_id = $this->input->post("prod_var_parent_id");
		$query = $this->db->query("SELECT prod_var_id FROM tb_prod_var WHERE 
		prod_var_parent_id = $prod_var_parent_id AND prod_var_parent_flag =2 
		");
		$child_no = $query->num_rows() + 1;
