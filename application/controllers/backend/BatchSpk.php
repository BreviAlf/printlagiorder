...
class BatchSpk extends CI_Controller {
	...
	public function AjaxInsertSPK()
	{
		...
		CreateLog('INFO','insert '.$batch_spk_id);
	...
	function cetak_batch()
	{
		...
		if ($ada_parent){
			...
			foreach ($arr_parent_id as $key => $value) {
			$sql_multiple_parent_id = $sql_multiple_parent_id . "spk_parent_id = " . $value . " OR ";
			}$sql_multiple_parent_id = substr($sql_multiple_parent_id,0,-3);
		...
		tb_spk
		WHERE
		spk_parent_flag = 2 AND ( $sql_multiple_parent_id )
		GROUP BY
		spk_material_name,spk_print_side 
		");
		...
		foreach ($arr_material as $material ) {
			...
			foreach  ($arr_child_material as $child) {
				if ($child['spk_material_name'] == $material['spk_material_name']){
					if ($child['spk_print_side'] == $material['spk_print_side']){
						// pair detected BEEB BOOP
						$arr_material[$i]['total'] = $arr_material[$i]['total'] + $child['total'];
						array_splice($arr_child_material, $j, 1);
					} else { 
						// echo $i.$j."VX";
					}
				} else { 
	...
	foreach  ($arr_child_material as $child) {
		...
		$qr_parent_material = $this->db->query("SELECT
		spk_material_name,spk_print_side, SUM(spk_qty_material) as total
		FROM
		tb_spk
		WHERE
		spk_parent_flag = 1 AND ( $sql_multiple_parent_id )
		GROUP BY
		spk_material_name,spk_print_side 
		");
		...
		foreach ($arr_material as $material ) {
			$j=0;
			foreach ($arr_parent_material as $parent ) {
				if ($parent['spk_material_name'] == $material['spk_material_name']){
					if ($parent['spk_print_side'] == $material['spk_print_side']){
						// pair detected BEEB BOOP
						$arr_material[$i]['total'] = $arr_material[$i]['total'] - $parent['total'];
						array_splice($arr_parent_material, $j, 1);
					} 
				} 
				$j++;
			}
		...
		// splice arr_material yang memiliki total 0
		$i = 0;
		foreach ($arr_material as $material ) {
			if($material['total'] <= 0){
				array_splice($arr_material, $i, 1);
			}
			$i++;
		}
		}
		...
		foreach ($arr_cutting as $cutting ) {
			$j=0;
			foreach ($arr_parent_cutting as $parent ) {
					if ($parent['spk_cutting'] == $cutting['spk_cutting']){
						// pair detected BEEB BOOP
						$arr_cutting[$i]['total'] = $arr_cutting[$i]['total'] - $parent['total'];
						array_splice($arr_parent_cutting, $j, 1);
					} 
				$j++;
			}
			$i++;
		...
		// print_r($arr_lam);
		if($ada_parent){
		$qr_child_lamination = $this->db->query("SELECT
		spk_lamination, SUM(spk_qty_material) as total
		FROM
		tb_spk
		WHERE
		spk_parent_flag = 2 AND spk_lamination <> 'Tanpa Laminasi' AND ( $sql_multiple_parent_id )
		GROUP BY
		spk_lamination 
		");

		$arr_child_lamination = $qr_child_lamination->result_array();
		// echo "<br><br>arr_child_lamination : <br>";
		// print_r($arr_child_lamination);
		// echo "<br>";



		$i = 0;
		foreach ($arr_lam as $lamination ) {
			$j = 0;
			foreach  ($arr_child_lamination as $child) {
				if ($child['spk_lamination'] == $lamination['spk_lamination']){
					$arr_lam[$i]['total'] = $arr_lam[$i]['total'] + $child['total'];
					array_splice($arr_child_lamination, $j, 1);
				} else { 
					// echo $i.$j."X";
				} 
		...
		foreach  ($arr_child_lamination as $child) {
			array_push($arr_lam, $child);
		}
		// echo '<br> after push child arr_lamination:<br>';
		// print_r($arr_lam);
		// echo '<br><br>';
		...
		// parent untuk cutting
		$qr_parent_lamination = $this->db->query("SELECT
		spk_lamination, SUM(spk_qty_material) as total
		FROM
		tb_spk
		WHERE
		spk_parent_flag = 1 AND spk_lamination <> 'Tanpa Laminasi' AND ( $sql_multiple_parent_id )
		GROUP BY
		spk_lamination
		");

		$arr_parent_lamination = $qr_parent_lamination->result_array();
		// echo "arr_parent_lamination : <br>";
		// print_r($arr_parent_lamination);
		// echo "<br>";
		...
		foreach ($arr_lam as $lamination ) {
			$j=0;
			foreach ($arr_parent_lamination as $parent ) {
					if ($parent['spk_lamination'] == $lamination['spk_lamination']){
						// pair detected BEEB BOOP
						$arr_lam[$i]['total'] = $arr_lam[$i]['total'] - $parent['total'];
						array_splice($arr_parent_lamination, $j, 1);
					} 
				$j++;
			}
			$i++;
		...
		$i=0;
		foreach ($arr_lam as $lamination ) {
			if($lamination['total'] <= 0){
				array_splice($arr_lam, $i, 1);
			}
			$i++;
		}
		// echo '<br> after pop parent arr_lam FINAL lamination :<br>';
		// print_r($arr_lam);
		// echo '<br><br>';
	}
		...
		*/
		...
		// Slice array_slice parent from $arr_finish
		...
		foreach ($arr_finish as $key => $value) {
			foreach ($arr_finish_parent as $key2 => $value2) {
				...
				if($key == $key2){
					$arr_finish[$key] = $value - $value2;
					unset($arr_finish_parent[$key]);
				}
				if($value - $value2 == 0){
					unset($arr_finish[$key]);
				}
	...
	function done_print()
	{
		...
		$data['title'] = 'SISPRINT - SCAN DONE';
		//sidebar prop
		$data['sidebar_active'] = "sidebar_bspk_print";
		$data['collapse_active'] = "bspk-nav";
		
		CreateLog('INFO','load page');
		$data['urut'] = $this->uri->segment(4);
		$data['template'] = 'batchspk/add_scan_print';
		$this->load->view('backend/index',$data);
	}

		//target2
	function AjaxInputscan_print()
	{
		$inputscan = trim($this->input->post('inputscan'));
			...
			else
			{
				...
				if($qr_scan_1->num_rows() != 0)
				{
					...
					if($qr_scan_2->num_rows() != 0)
					{
						...
						$data_update_batch_detail = array
						(
							'batch_spk_det_print'			=> $time_now,
							'batch_spk_det_material_qty'	=> $qty,
							'batch_spk_det_print_user_id'	=> $this->session->userdata('user_id')
						);
