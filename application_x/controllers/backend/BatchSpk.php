...
class BatchSpk extends CI_Controller {
	...
	public function AjaxInsertSPK()
	{
		...
		if($data_spk)
		{
			...
			else
			{
				...
				copyDirectory(GetImg($data_spk->spk_inv_mp,$batch_spk_no),GetFolderBatch($batch_name).'\\'.$data_spk->spk_inv_mp.'\\'.$batch_spk_no,$data_spk->spk_inv_mp);
	...
	public function AjaxDelDetSPKId()
	{
		...
		$src = GetFolderBatch($batch_name).'\\'.$row_det->batch_spk_det_spk_inv_mp.'\\'.$row_det->batch_spk_det_spk_no;
		$dst = GetImg($row_det->batch_spk_det_spk_inv_mp,$row_det->batch_spk_det_spk_no);
		copyDirectory($src,$dst,$row_det->batch_spk_det_spk_inv_mp);

		/*
		$del_notif = "";
		$iterator = new \FilesystemIterator(GetFolderInv($row->spk_inv_mp));
		$isDirEmpty = !$iterator->valid();
		if($isDirEmpty){
			$del_notif =  'REMOVE ' .GetFolderInv($row->spk_inv_mp);
			rmdir(GetFolderInv($row->spk_inv_mp));
		}
		*/
